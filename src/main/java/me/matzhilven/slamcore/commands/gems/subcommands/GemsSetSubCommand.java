...
public class GemsSetSubCommand implements SubCommand {

    private final SlamCore main;

    public GemsSetSubCommand(SlamCore main) {
        this.main = main;
    }
    ...
    public void onCommand(CommandSender sender, Command command, String[] args) {
        if (args.length != 3) {
            StringUtils.sendMessage(sender, Messager.SET_GEMS_USAGE);
            return;
        }
        ...
        StringUtils.sendMessage(sender, Messager.SET_GEMS_SENDER.replace("%target%", target.getName()).replace("%gems%", StringUtils.format(gems)));
        StringUtils.sendMessage(target, Messager.SET_GEMS.replace("%gems%", StringUtils.format(gems)));
