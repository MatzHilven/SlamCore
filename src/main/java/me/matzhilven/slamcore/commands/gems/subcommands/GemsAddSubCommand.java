...
public class GemsAddSubCommand implements SubCommand {

    private final SlamCore main;

    public GemsAddSubCommand(SlamCore main) {
        this.main = main;
    }
    ...
    public void onCommand(CommandSender sender, Command command, String[] args) {
        if (args.length != 3) {
            StringUtils.sendMessage(sender, Messager.ADD_GEMS_USAGE);
            return;
        }
        ...
        StringUtils.sendMessage(sender, Messager.ADD_GEMS_SENDER.replace("%target%", target.getName()).replace("%gems%", StringUtils.format(gems)));
        StringUtils.sendMessage(target, Messager.ADD_GEMS.replace("%gems%", StringUtils.format(gems)));
